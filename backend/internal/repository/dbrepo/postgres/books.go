...
func (b booksRepository) Create(ctx context.Context, book *entity.Book) error {
	...
	err = tx.QueryRow(ctx, bookInsertQuery, book.Title, book.Subtitle, book.Authors, book.Publisher, book.Description, book.PageCount, book.Categories, book.Language, book.ImageLinks.SmallThumbnail, book.ImageLinks.Thumbnail).Scan(&book.ID)
	if err != nil {
		tx.Rollback(ctx)
		return err
	}
